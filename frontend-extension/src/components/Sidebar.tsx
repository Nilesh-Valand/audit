...
export function Sidebar() {
  ...
  // Settings writes directly to localStorage, so re-read on route change
  // (cheap) to keep the footer in sync without a global store.
  useEffect(() => {
    setHost(backendHost());
  }, [pathname]);
    ...
    <aside className="sticky top-0 z-20 flex h-screen w-72 shrink-0 flex-col self-start border-r border-slate-200 bg-white text-slate-700">
      ...
      <div className="border-t border-slate-200 px-5 py-4">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-xs font-medium text-slate-600">Extension workspace</div>
            <div className="truncate text-[11px] text-slate-400" title={getApiBaseUrl()}>
              Backend · {host}
            </div>
          </div>
