/**
 * Browser persistence replacing chrome.storage.local from the extension.
 *
 * The extension never stored an auth token/session — only API base URL and
 * selected audit IDs. FastAPI also has no auth cookie endpoints today.
 * Choice: localStorage (client-readable prefs; no httpOnly cookie to set).
 */
